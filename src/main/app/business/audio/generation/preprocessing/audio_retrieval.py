...
def retrieve_via_path(path: str, required_sample_rate: float) -> tuple[ndarray, float]:
    audio_data, sr = librosa.load(path)
    return _normalize(audio_data, sr, required_sample_rate), required_sample_rate


def retrieve_via_storage(storage: FileStorage, required_sample_rate: float) -> tuple[ndarray, float]:
    audio_data, sr = librosa.load(storage)
    return _normalize(audio_data, sr, required_sample_rate), sr


def _normalize(audio_data, sr: int, required_sample_rate: float) -> ndarray:
    # Normalize the audio
